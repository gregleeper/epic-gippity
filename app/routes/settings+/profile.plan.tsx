...
export async function action({ request }: ActionFunctionArgs) {
	...
	if (intent === 'resumeSubscription') {
		try {
			...
			return redirectWithToast('/settings/profile/plan', {
				type: 'success',
	...
	if (intent === 'createSubscription') {
		...
		const session = await stripe.checkout.sessions.create({
			...
			mode: 'subscription',
			success_url: `${process.env.BASE_URL}/settings/profile/plan-success?session_id={CHECKOUT_SESSION_ID}`,
			cancel_url: `${process.env.BASE_URL}/settings/profile/plan`,
		})
